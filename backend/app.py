...
## 可行性评估
这个实验方案设计合理，材料易得，步骤清晰，适合中学生操作。

## 科学性检查
实验原理正确，符合物理规律，数据记录方法科学。
...
**改进时间：{time.strftime('%Y-%m-%d %H:%M')}**"""
        else:
            # 生成方案
            topic_match = re.search(r'主题「([^」]+)', prompt)
            topic = topic_match.group(1) if topic_match else "物理实验"
            
            return f"""# {topic} 创新实验方案
